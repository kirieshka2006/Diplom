...
class Booking(models.Model):
    ...
    # ★★★ ДОБАВЬ ЭТИ ПОЛЯ ★★★
    booking_full_name = models.CharField(max_length=255, verbose_name="ФИО для брони", blank=True)
    booking_email = models.EmailField(verbose_name="Email для уведомлений", blank=True)
    booking_phone = models.CharField(max_length=20, verbose_name="Телефон для связи", blank=True)
...
class ReviewReply(models.Model):
    ...
    def __str__(self):
        return f"Ответ на отзыв {self.review.id} от {self.user.username}"
